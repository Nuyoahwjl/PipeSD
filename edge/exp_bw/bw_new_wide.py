...
def plot_bar(metric, ylabel, out_name):
    plt.figure(figsize=(10, 3.6), dpi=900)

    bar_width = 0.3
    group_gap = 0.2
    x = np.arange(len(bandwidths_mbps)) * (len(methods) * bar_width + group_gap)
...
plot_bar(
    metric="avg_tpt",
    ylabel="Avg. TPT (ms)",
    out_name="figs/avg_tpt.png",
)

plot_bar(
    metric="gpu_power_int",
    ylabel="Avg. ECS (J)",
    out_name="figs/avg_ecs.png",
)
